...
public:
  NoiseVideoSource()
      : VideoSource(VideoSourceConfiguration::create(false, true)),
        m_stopped(false), m_thread(&NoiseVideoSource::run, this) {}
...
private:
  void run() {
    ...
    while (!m_stopped.load()) {
      ...
      int64_t timestampUs = chrono::duration_cast<chrono::microseconds>(
...
public:
  SinAudioSource()
      : AudioSource(AudioSourceConfiguration::create(SoundCardTotalDelayMs),
        ...
        m_stopped(false), m_thread(&SinAudioSource::run, this) {}
...
private:
  void run() {
    vector<int16_t> data(
        SinAudioSourceFrameDuration.count() * SampleRate / 1000, 0);
    double t = 0;
    ...
    while (!m_stopped.load()) {
      ...
      this_thread::sleep_for(SinAudioSourceFrameDuration -
                             SinAudioSourceSleepBuffer);
      while ((chrono::steady_clock::now() - start) <
             SinAudioSourceFrameDuration)
        ;
    }
  }
};
...
int main(int argc, char *argv[]) {
  ...
  if (!IceServer::fetchFromServer("http://localhost:8080/iceservers", "abc",
                                  iceServers)) {
  ...
  auto signalingServerConfiguration = SignalingServerConfiguration::create(
      "http://localhost:8080", "C++", "chat", "abc");
  ...
  client.setOnAudioFrameReceived(
      [](const Client &client, const void *audioData, int bitsPerSample,
         int sampleRate, size_t numberOfChannels, size_t numberOfFrames) {
        // This callback is called from a WebRTC processing thread.
  ...
  client.setOnMixedAudioFrameReceived(
      [](const void *audioData, int bitsPerSample, int sampleRate,
