...
static vector<webrtc::SdpVideoFormat> filterSupportedFormats(const vector<webrtc::SdpVideoFormat>& supportedFormats,
                                                             const unordered_set<VideoStreamCodec>& forcedCodecs)
{
    ...
    copy_if(supportedFormats.begin(),
...
static typename Factory::CodecSupport queryCodecSupport(const unique_ptr<Factory>& factory,
...
ForcedCodecVideoDecoderFactory::ForcedCodecVideoDecoderFactory(
    ...
    unordered_set<VideoStreamCodec> forcedCodecs) : m_factory(move(factory)), m_forcedCodecs(move(forcedCodecs))
{
}
...
webrtc::VideoDecoderFactory::CodecSupport ForcedCodecVideoDecoderFactory::QueryCodecSupport(
    const webrtc::SdpVideoFormat& format,
...
unique_ptr<webrtc::VideoDecoder> ForcedCodecVideoDecoderFactory::CreateVideoDecoder(const webrtc::SdpVideoFormat& format)
{
    return m_factory->CreateVideoDecoder(format);
}


ForcedCodecVideoEncoderFactory::ForcedCodecVideoEncoderFactory(unique_ptr<webrtc::VideoEncoderFactory> factory,
                               unordered_set<VideoStreamCodec> forcedCodecs) : m_factory(move(factory)), m_forcedCodecs(move(forcedCodecs))
{
}
...
unique_ptr<webrtc::VideoEncoder> ForcedCodecVideoEncoderFactory::CreateVideoEncoder(const webrtc::SdpVideoFormat& format)
{
...
unique_ptr<webrtc::VideoDecoderFactory> opentera::createVideoDecoderFactory(const VideoStreamConfiguration& configuration)
{
    ...
    return make_unique<ForcedCodecVideoDecoderFactory>(move(gstreamerVideoDecoderFactory),
                                                       configuration.forcedCodecs());
}

unique_ptr<webrtc::VideoEncoderFactory> opentera::createVideoEncoderFactory(const VideoStreamConfiguration& configuration)
{
    // TODO change to GStreamer
    return make_unique<ForcedCodecVideoEncoderFactory>(webrtc::CreateBuiltinVideoEncoderFactory(),
                                                       configuration.forcedCodecs());
}
...
unique_ptr<webrtc::VideoDecoderFactory> opentera::createVideoDecoderFactory(const VideoStreamConfiguration& configuration)
{
    return make_unique<ForcedCodecVideoDecoderFactory>(webrtc::CreateBuiltinVideoDecoderFactory(),
                                                       configuration.forcedCodecs());
}
