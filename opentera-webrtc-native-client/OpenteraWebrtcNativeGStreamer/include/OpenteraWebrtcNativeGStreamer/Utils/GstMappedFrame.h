...
namespace opentera
{
    ...
    inline uint8_t* GstMappedFrame::componentData(int comp) { return m_isValid ? GST_VIDEO_FRAME_COMP_DATA(&m_frame, comp) : nullptr; }
    inline int GstMappedFrame::componentStride(int stride) { return m_isValid ? GST_VIDEO_FRAME_COMP_STRIDE(&m_frame, stride) : -1; }
    ...
    inline int GstMappedFrame::format() { return m_isValid ? GST_VIDEO_FRAME_FORMAT(&m_frame) : GST_VIDEO_FORMAT_UNKNOWN; }
}
